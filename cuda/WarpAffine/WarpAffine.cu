...
int main() {
	...
	while (cv::waitKey(1) != 'q') {
		...
		const int shift_x = 60;
		const cv::Point2f src_pt[] = { cv::Point2f(0, 0), cv::Point2f(0, 480), cv::Point2f(640, 0) };
		const cv::Point2f dst_pt[] = { cv::Point2f(0 + shift_x, 0), cv::Point2f(0 + shift_x, 480), cv::Point2f(640 + shift_x, 0) };
		const cv::Mat affine_matrix = cv::getAffineTransform(src_pt, dst_pt);
		std::cout << "affine_matrix = " << affine_matrix << std::endl;
		cv::warpAffine(color, color, affine_matrix, color.size(), cv::INTER_LINEAR);
